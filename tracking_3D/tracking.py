...
class Tracker(object):
    ...
    def track_iteration_online(self, frame_id, frame_current):
        ...
        instances = self.detection_postprocess(instances)
        '''
        f= open(os.path.join(r"/scratch/rparikh4/tracking_3d/maskrcnn_txt/","{:06d}".format(frame_id)+'.txt'),"w")
        for instance in instances:
            box = instance.box
            score = instance.score.tolist()
            txt=['Car',-1,-1,-10]
            txt+=[val for val in box.tolist()]
            txt+=[-1,-1,-1,-1000,-1000,-1000,-10]
            txt.append(score)
            #print(txt)
            for element in txt:
    ...
    def compute_3d_states(self, camera_model, map_model, instances, frame_id):
        ...
        print("Shape BBox 3D: ", len(bbox_3d)/8)
        print("Shape of Instances: ", len(instances))
        ...
        for i, instance in enumerate(instances):
            ...
            for k in range(len(final_pbox)):
                cv2.circle(image, (int(final_pbox[k][0]), int(final_pbox[k][1])), 1, (255,255,255), -1)
            cv2.imwrite(os.path.join("/scratch/rparikh4/tracking_3d/intermediate_annotated_frames/"+str(frame_id) + "_" + str(i) + ".png"),image)
            ...
            bb3d = bb3d_perspective(final_pbox, camera_model, instance.contour,
...
def pipeline_visualize(tracker, frame_rect, frame_id, fvis, mvis, rot_map, h_line, writer, render=False):
    ...
    cv2.imwrite('/scratch/rparikh4/tracking_3d/final_frames/'+str(frame_id)+".png", frame_vis3)
    # frame_vis1[0:36, 0:] = 0
    # frame_vis2[0:36, 0:] = 0
    # frame_vis3[0:36, 0:] = 0
...
def test_tracker(cam_id, track_id, folder, prefixes):
    ...
    for frame_id in tqdm(range(init_frame_id + 1, 1720)):

        ret, frame = cap.read()
        cv2.imwrite(os.path.join("/scratch/rparikh4/tracking_3d/frames/",'{:06d}'.format(frame_id) + ".png"),frame)
        if not ret:
