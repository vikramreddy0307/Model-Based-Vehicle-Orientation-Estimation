...
def bb3d_perspective(bbox_3d, pbox, centroids,cm, contour, vpx, vpy, vpz, vx, vy, vz):
    ''' 
    ...
    if situation == 1:
        ...
        nbox_ori = bb3d_nbox_xvpp(
            lx0, lx1, ly0, ly1, lz0, lz1, vpx, vpy, vpz)
        nbox_ori = np.array(nbox_ori)
        if not nbox_ori:
            return None
        nbox_centroid = np.mean(nbox_ori,axis=0)   
        #print("N Box Ori centroid: ", nbox_centroid)
    
        dist = []
        min_index = 0
        mini = float('inf')
        for i in range(centroids.shape[0]):
            dist.append(np.linalg.norm(centroids[i] - nbox_centroid))
            if dist[i]<mini:
                mini = dist[i]
                min_index = i
        
        #print("Distances b/w centroid: ",dist)
        #print("Centroid Chosen: ", min_index, "Centroid: ",centroids[min_index])
        #nbox = bbox_3d[min_index*8:(min_index+1)*8]
        cases, pbox, visibility = bb3d_pbox_xvpp(nbox, vx, vy, vz)
    
    
    elif situation == 2:
        
        nbox_ori = bb3d_nbox_yvpp(
            lx0, lx1, ly0, ly1, lz0, lz1, vpx, vpy, vpz)
